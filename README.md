...
| 팀원               | 소감                                                  |
|--------------------|------------------------------------------------------------|
| 김혜원 | 데이터 전처리 과정에서 다수의 결측값을 단순히 제거하는 대신, 데이터의 패턴을 파악해 합리적으로 보완하는 경험이 인상 깊었다.<br>결측값이 발생한 맥락과 분포를 EDA를 통해 먼저 확인한 뒤, 데이터의 특성에 맞는 방식으로 값을 채우면서 전처리가 단순한 기술적 작업이 아니라 해석이 필요한 과정임을 깨달았다.<br>또한 파생 변수를 생성할 때 임의로 기준을 설정하지 않고, 관련 도메인을 조사해 의미 있는 기준을 정의했다는 점이 중요하게 느껴졌다.<br>RFM 분석을 위해 각 지표에 적합한 컬럼을 직접 정의하고, 이를 기반으로 세그먼트 기준을 설정해 총 11개의 고객 세그먼트를 분류한 경험 역시 새로웠다.<br>더 나아가 세그먼트 간 유사한 특성을 정량적으로 확인하기 위해 K-means 클러스터링을 적용함으로써, 주관적 판단이 아닌 데이터에 근거한 그룹화를 수행할 수 있었다는 점에서 의미 있는 분석이라고 느꼈다.      |
