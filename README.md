...
| **RFM 기반 고객 세그먼트 분류** | - **RFM_Segment** 컬럼을 기준으로 고객을 유형별로 세그먼트 분류<br> - 세그먼트 분류 기준은 [Bloomreach RFM Segmentation 기준](https://documentation.bloomreach.com/engagement/docs/rfm-segmentation)을 참고하여 정의 <br>- 11개 고객 세그먼트 유형 : 이탈 고객, 동면 고객, 놓치면 안 될 고객, 이탈 우려 고객, 휴면 예정 고객, 관심 필요 고객, 잠재 고객, 신규 고객, 잠재 충성 고객, 충성 고객, VIP 고객                        |
...
| 팀원               | 소감                                                  |
|--------------------|------------------------------------------------------------|
| 김혜원 | 데이터 전처리 과정에서 다수의 결측값을 단순히 제거하는 대신, 데이터의 패턴을 파악해 합리적으로 보완하는 경험이 인상 깊었다.<br>결측값이 발생한 맥락과 분포를 EDA를 통해 먼저 확인한 뒤, 데이터의 특성에 맞는 방식으로 값을 채우면서 전처리가 단순한 기술적 작업이 아니라 해석이 필요한 과정임을 깨달았다.<br><br>또한 파생 변수를 생성할 때 임의로 기준을 설정하지 않고, 관련 도메인을 조사해 의미 있는 기준을 정의했다는 점이 중요하게 느껴졌다.<br><br>RFM 분석을 위해 각 지표에 적합한 컬럼을 직접 정의하고, 이를 기반으로 세그먼트 기준을 설정해 총 11개의 고객 세그먼트를 분류한 경험 역시 새로웠다.<br>더 나아가 세그먼트 간 유사한 특성을 정량적으로 확인하기 위해 K-means 클러스터링을 적용함으로써, 주관적 판단이 아닌 데이터에 근거한 그룹화를 수행할 수 있었다는 점에서 의미 있는 분석이라고 느꼈다.      |
| 박찬우 | 단순히 클러스터링을 돌리는 것보다 파생변수 설계와 merge 기준을 정하는 과정이 성능과 해석을 좌우한다는 걸 체감했다.  RFM 11개 세그먼트, K-means 3개 군집, 코호트 분석을 통해 고객의 가치와 유지 패턴을 입체적으로 볼 수 있었다.  데이터로 고객을 설명하고 실제 마케팅 실행안까지 설계해본 것이 가장 큰 수확이었다.      |
